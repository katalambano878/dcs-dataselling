...
class QueryBuilder implements PromiseLike<{ data: any; error: any; count: number | null }> {
  ...
  private async resolveEmbeds(
    rows: Row[],
    parsed: ParsedSelect,
    /** Table the `rows` belong to (needed for nested embeds). */
    fromTable: string = this.table,
  ): Promise<void> {
    if (rows.length === 0 || parsed.embeds.length === 0) return;
    const pool = getPool();
    for (const embed of parsed.embeds) {
      // If no explicit FK column, decide direction from the FK map: an edge
      // from the current row table to the embed table means forward (object);
      ...
      if (fk) {
        ...
        const own = (FK_MAP[fromTable] || []).find((e) => e.column === fk);
        if (own) embedTable = own.foreignTable;
      } else {
        const fwd = (FK_MAP[fromTable] || []).find((e) => e.foreignTable === embed.table);
        if (fwd) fk = fwd.column;
      }
      if (fk) {
        ...
        // Nested embeds must resolve against the related table, not the root query table
        ...
        const edge = (FK_MAP[embedTable] || []).find((e) => e.foreignTable === fromTable);
        const fkCol = edge?.column ?? `${singularize(fromTable)}_id`;
  ...
  private embedColumns(
    parsed: ParsedSelect,
    extraFk?: string,
    /** Table these columns are selected from (so nested embed FKs can be added). */
    fromTable?: string,
  ): string {
    if (parsed.star && parsed.columns.length === 0) return "*";
    const cols = new Set<string>();
    ...
    for (const e of parsed.embeds) {
      ...
      const fwd = (FK_MAP[fromTable] || []).find((x) => x.foreignTable === e.table);
...
export type CompatAuthOptions = {
  /** Resolve the current access JWT (cookies, Authorization header, etc.). */
  getAccessToken?: () => string | null | Promise<string | null>;
};

function createAuthApi(opts: CompatAuthOptions = {}) {
  ...
  async function resolveToken(explicit?: string): Promise<string | null> {
    ...
    if (opts.getAccessToken) {
      return (await opts.getAccessToken()) ?? null;
    }
    return null;
  }
  ...
  return {
    ...
    async getUser(jwt?: string) {
      const token = await resolveToken(jwt);
      if (!token) {
    ...
    async updateUser(attrs: { password?: string; data?: Record<string, unknown> }) {
      ...
      if (attrs.password) {
        const { user, error } = await auth.updateUserPassword(verified.userId, attrs.password);
        if (error) {
      ...
      if (attrs.data) {
        const { user, error } = await auth.updateUserMetadata(verified.userId, attrs.data);
        if (error) {
          return { data: { user: null }, error: { message: error } };
        }
        return { data: { user }, error: null };
      }
      return { data: { user: null }, error: { message: "No attributes to update" } };
    },
    onAuthStateChange() {
    ...
    admin: {
      async createUser(opts: {
        ...
        const auth = await loadAuth();
        const { user, error } = await auth.adminCreateUser(opts);
        if (error) {
          return { data: { user: null }, error: { message: error } };
        }
        return { data: { user }, error: null };
      ...
      async updateUserById(userId: string, attrs: { password?: string }) {
        const auth = await loadAuth();
        if (attrs.password) {
          const { user, error } = await auth.updateUserPassword(userId, attrs.password);
          if (error) {
            return { data: { user: null }, error: { message: error } };
          }
          return { data: { user }, error: null };
        ...
        return {
          data: { user: null },
          error: { message: "updateUserById requires supported attributes" },
        };
      },
    },
  };
...
export function createClient(
  _urlOrOpts?: string | CompatAuthOptions,
  _key?: string,
  maybeOpts?: CompatAuthOptions,
): SupabaseCompatClient {
  const opts: CompatAuthOptions =
    typeof _urlOrOpts === "object" && _urlOrOpts !== null
      ? _urlOrOpts
      : maybeOpts ?? {};
