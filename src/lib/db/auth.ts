...
export async function adminCreateUser(opts: {
  ...
  const email = opts.email.trim().toLowerCase();
  if (!email || !opts.password || opts.password.length < 6) {
    return { user: null, error: "Email and password (min 6 chars) required" };
  }
  ...
  await query(
    `INSERT INTO auth.users (
       ...
       $1, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated',
       $2, $3, $5, '{"provider":"email","providers":["email"]}'::jsonb, $4::jsonb,
       now(), now(), '', '', '', ''
     )`,
    [id, email, hash, JSON.stringify(meta), opts.email_confirm ? new Date().toISOString() : null]
  );
