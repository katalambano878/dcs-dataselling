...
export function SupplierPollShopDcsButton({ disabled }: { disabled?: boolean }) {
  ...
  async function poll() {
    try {
      const res = await fetch("/api/admin/supplier/poll-shopdcs", { method: "POST" });
      ...
      setLast(
        `Polled ${data.polled ?? 0} · fulfilled ${data.fulfilled ?? 0} · failed ${data.failed ?? 0} · pending ${data.stillProcessing ?? 0}`,
      );
      toast.success("Shop DCS status sync complete");
